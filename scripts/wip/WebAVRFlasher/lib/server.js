...
server.post('/flash', function (req, res) {
	...
	//var flashResults = exec('avrdude -v -c arduino -p ATMEGA328P -P /dev/ttyAMA0 -b 115200 -U flash:w:firmware.hex').toString();
	var flashResults = exec('sh lib/flash.sh').toString();
	console.log(flashResults.indexOf("flash verified") > -1);
	console.log(flashResults.length);
